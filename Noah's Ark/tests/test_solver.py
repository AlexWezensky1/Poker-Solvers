...
def test_eight_cards_pick_the_best_five():
    """Two hole cards against six community cards is eight to choose from,
    one more than Hold'em ever hands the evaluator."""
    rng = random.Random(99)
    deck = list(FULL_DECK)
    for _ in range(400):
        eight = rng.sample(deck, 8)
        best = max(score(five) for five in combinations(eight, 5))
        assert score(eight) == best, eight
...
def test_a_dealt_board_is_enumerated_exactly():
    """Four community cards down leaves two to come, so the walk is tiny."""
    report = equity([parse_cards("AsKs"), parse_cards("QhQd")],
                    parse_cards("Jh Ts 2c 5d"))
    assert report.exact
    assert report.trials == 946  # C(44, 2)
...
def test_preflop_pair_versus_pair():
    """Six community cards give the underdog two extra draws, so aces over
    kings is a good deal thinner here than the ~82% it is in Hold'em."""
    report = equity(
        [parse_cards("AsAd"), parse_cards("KsKh")],
        trials=120_000, seed=11,
    )
    assert not report.exact
    assert abs(report.hands[0].equity_pct - 77.8) < 1.0, report.hands[0].equity_pct


def test_preflop_pair_versus_two_overcards():
    """The Hold'em coinflip lands even flatter here: two more community cards
    are two more chances for the overcards to pair."""
    report = equity(
        [parse_cards("QhQd"), parse_cards("AsKs")],
        trials=120_000, seed=13,
    )
    assert abs(report.hands[0].equity_pct - 50.3) < 1.0, report.hands[0].equity_pct


def test_board_holds_six_cards():
    hands = [parse_cards("AsKs"), parse_cards("QhQd")]
    equity(hands, parse_cards("Jh Ts 2c 5d 8h 9c"))  # a full board is fine
    try:
        equity(hands, parse_cards("Jh Ts 2c 5d 8h 9c 4s"))
    except ValueError as exc:
        assert "at most 6" in str(exc), exc
    else:
        raise AssertionError("a seventh community card should be refused")


def test_forcing_the_walk_falls_back_when_it_is_too_wide():
    """C(48,6) is twelve million runouts; it samples rather than sit there."""
    report = equity([parse_cards("AsKs"), parse_cards("QhQd")],
                    mode="exact", trials=5_000, seed=3)
...
def test_made_odds_cover_every_runout():
    ...
    for hand in report.hands:
        assert abs(sum(pct for _, pct in hand.made_pct) - 100.0) < 1e-9
