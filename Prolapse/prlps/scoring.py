"""Hand scoring and pot resolution for Prolapse.
...
1. Anyone who empties their hand *before* the last street wins outright.
...
2. Otherwise the last street is turned and the pot goes to everyone who
   either emptied their hand on it or still holds all six cards.  Those two
...
def profile_for_ranks(ranks):
    ...
    hole cards leans on that: there are only a few thousand distinct six card
...
def showdown(hand_masks, tables, board):
    """Settle the pot once the last street is out.

    ``board`` is the rank mask of all eleven community cards.  Returns
    ...
    # Going out on the last street no longer wins outright, it only earns a share
