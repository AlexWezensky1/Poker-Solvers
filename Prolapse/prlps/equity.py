"""Equity calculation for Prolapse.
...
#: Eleven community cards plus six apiece caps the table at six.
    ...
    def fill(street, state, board, spare):
        ...
        for taken, ways, added in _street_draws(state, bits, need, spare):
            ...
            if street < LAST_STREET:
                ...
                if gone:
                    # Out before the last street, so the hand ends here
...
def equity(hands, board=(), discards=None, dead=(), trials=DEFAULT_TRIALS,
           ...
           seconds=None):
    """Compute equity for two or more Prolapse hands.
    ...
    card ints from :mod:`prlps.cards`.  Everyone is dealt five, so whatever the
    ...
    lying face up rather than hidden.  ``board`` holds 0-11 community cards in
    dealing order.
