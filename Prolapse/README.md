...
Equity calculator for Prolapse. Give it up to six six-card hands and up to eleven
...
```bash
python -m prlps AsKsQsJsTs 2h3h4h5h6h --board "2c3c4c5c 7h8h9h"
```
...
Everyone is dealt six cards and four community cards go straight out. Any card
...
cards, then three, then two, then two.
...
```
python -m prlps [HAND ...] [-b BOARD] [-t TRIALS] [-m MODE] [--seed N] [--json]
```

| Option | Meaning |
| --- | --- |
| `HAND ...` | up to 8 hands, `HELD` or `HELD/DISCARDED`; run with none to be prompted |
| `-b`, `--board` | 0-11 community cards in dealing order |
...
```bash
python -m prlps KsKhQsQhJs AcKdQdJdTd                      # opening deal, sampled
python -m prlps KsKhQsQhJs AcKdQdJdTd --mode exact         # walked in full
python -m prlps AsKsQsJsTs 2h3h4h5h6h -b "2c3c4c5c" --json
python -m prlps AsKsQsJsTs /2c3c -b "2s3s4s7s"             # villain read only by discards
python -m prlps                                            # prompts for hands
```
...
Bad input comes back as a `400` with a plain english `detail`. `GET /prlps/api/health`
is a liveness probe. Interactive API docs are at `/prlps/api/docs`.

## How it works

**Suits do not exist.** Nothing in Prolapse reads a suit — community cards match by
...
Both engines settle through the same `prlps.scoring.resolve`, so the rules live in
exactly one place.
