"""FastAPI front end for the Prolapse equity solver.
...
from prlps.cards import BOARD_SIZE, HAND_SIZE, cards_str, parse_cards
from prlps.equity import DEFAULT_TRIALS, MAX_PLAYERS, equity
...
app = FastAPI(title="Prolapse Solver", docs_url="/prlps/api/docs", redoc_url=None)
...
class EquityRequest(BaseModel):
    ...
    board: str = Field("", description="0-11 community cards in dealing order")
    dead: str = Field(
        "",
...
@app.post("/prlps/api/equity", response_model=EquityResponse)
...
@app.get("/prlps/range", include_in_schema=False)
def range_chart():
...
def index():
    """The solver lives under /prlps; keep the bare domain pointing at it."""
    return RedirectResponse("/prlps/")


app.mount("/prlps", StaticFiles(directory=STATIC_DIR, html=True), name="static")
