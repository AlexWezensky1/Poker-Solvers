"""Tests for the Prolapse scoring rules and equity engines.
...
from prlps.cards import (  # noqa: E402
    FULL_DECK, HAND_SIZE, RANK_BIT, RANK_CHARS, STREETS, parse_cards, rank_mask,
)
from prlps.equity import equity  # noqa: E402
from prlps.scoring import (  # noqa: E402
...
def test_lone_ace_scoops_lone_jack():
    ...
    hands = [parse_cards("As2h3h5h7h4h"), parse_cards("Jh2d3d5d7d4d")]
    board = parse_cards("2s2c3s3c5s5c7cTh9h8h4s")
    assert settle(hands, board) == [1.0, 0.0]


def test_high_low_ties_split_their_own_half():
    """4 vs 4 vs 36 pays 25 / 25 / 50."""
    hands = [parse_cards("4s2h3h5h7h9c"), parse_cards("4h2d3d5d7d9d"),
             parse_cards("KsQsJs6s7s9s")]
    board = parse_cards("2s2c3s3c5s5c7cTh9h8hTc")
...
def test_a_pot_won_outright_counts_in_neither_half():
    """Out before the last street takes the lot, which is never split in two."""
    hands = [parse_cards("KsKhQsQhJsJh"), parse_cards("2h2d3h3d4h4d")]
    board = parse_cards("KcQcJcTs" "2c3c4c" "5s6s" "7s8s")
...
def test_simultaneous_outs_split():
    hands = [parse_cards("KsKhKdQsQhQd"), parse_cards("2h2d2c3h3d3c")]
    board = parse_cards("KcQc2s3s" "7s8s9s" "TsJs" "4s5s")
    assert settle(hands, board) == [0.5, 0.5]


def test_out_on_last_card_with_no_keeper_takes_everything():
    hands = [parse_cards("KsKhKdQsQhQd"), parse_cards("2h3h4h5h6h")]
...
def test_exact_matches_brute_force():
    # Boards are left one or two cards short of the eleven, which is what keeps
    # the brute force walk small enough to be worth having.
    cases = [
        (["AsKsQsJsTs9s", "2h3h4h5h6h7h"], "2c3c4c5c7d8d9dJdQdKd"),
        (["AsKsQsJsTs9s", "2h3h4h5h6h7h"], "2c3c4c5c7d8d9dJdQd"),
        (["KsKhQsQhJsJh", "2h3h4h5h6h7h"], "Ac3c4c5c7d8d9dTdJd"),
        (["AsAhKsKhQsQh", "2h2d3h3d4h4d"], "Tc9c8c7c6h5d4c3c2c"),
    ]
...
def test_pairs_beat_broadway():
    ...
    report = equity(
        [parse_cards("KsKhQsQhJsJh"), parse_cards("AcKdQdJdTd9d")],
...
def test_describe_lists_every_card_a_pair_included():
    ...
    hands = [parse_cards("KsKhQsJdJs8c"), parse_cards("3s2s4s5sJh8d")]
    board = parse_cards("KdQdTs9s" "7s8s8h" "7h7d" "3h9h")
    report = equity(hands, board)
...
def test_discards_are_just_part_of_the_hand():
    ...
    board = parse_cards("2s2c3s3c5s5c7cTh9h8h4s")
    whole = equity([parse_cards("As2h3h5h7h4h"), parse_cards("Jh2d3d5d7d4d")], board)
    split = equity(
        [parse_cards("As"), parse_cards("Jh")], board,
        discards=[parse_cards("2h3h5h7h4h"), parse_cards("2d3d5d7d4d")],
    )
...
def test_unknown_cards_cannot_be_ones_the_board_already_matched():
    ...
    The board covers ranks 2-7 exactly. Hero holds a lone ace after five
    discards; the villain has five discards and one unknown card. That card can
    ...
    27 cards could still be hidden and 3 of them are aces, so hero's equity is
    exactly (24 + 3 * 0.5) / 27.
    """
    board = parse_cards("2s2h3s3h" "4s4h5s" "5h6s" "6h7s")
    report = equity(
        [parse_cards("Ah"), []], board,
        discards=[parse_cards("2d3d4d5d6d"), parse_cards("2c3c4c5c6c")],
        trials=100_000, seed=17,
    )
    assert report.hands[1].unknown == 1
    assert report.hands[1].out_pct == 0.0
    expected = 100.0 * (24 + 3 * 0.5) / 27
...
def test_exact_walks_unknown_cards_when_few_enough_ways_to_deal_them():
    ...
    hands = [parse_cards("QsQhJdJsThTc"), ()]
    discards = [(), parse_cards("KcQc")]
...
def test_walking_the_unknown_agrees_with_naming_it():
    ...
    board = parse_cards("KdQdTs9s")
    known = parse_cards("QsQhJdJsThTc")
    partial = parse_cards("8c7c6c5c4c")
...
def test_rejects_bad_hands():
    good = parse_cards("AsKsQsJsTs9s")
    for hands, board, discards, expected in (
        ([good], (), None, "at least 2"),
        ([good, parse_cards("2h3h4h5h6h7h8h")], (), None, "more than the 6"),
        ([good, parse_cards("AsKsQsJsTs9s")], (), None, "twice"),
        ([good, parse_cards("2h3h4h5h6h7h")],
         parse_cards("2d3d4d5d6d7d8d9dTdJdQdKd"), None, "at most 11"),
