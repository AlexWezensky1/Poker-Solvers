"""Showdown chart for Prolapse: every hand shape against every other, by top card.

Suits are irrelevant in Prolapse -- ``prlps.cards`` says so outright, nothing in
the game ever reads one -- and the order of cards within a hand does not matter
either. A hand is therefore a multiset of six ranks, and there are 18,395 of
them once no rank may appear more than four times.

That is 169,178,815 unordered pairs, and they are dear ones: six card hands put
more live ranks in play than five, and the board runs to eleven, so an exact
preflop matchup measured at about two and a half minutes. The whole pairing is
therefore centuries of single core work, and this script does not attempt it.
...
# ``prlps`` sits one level up; the script runs from anywhere.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from prlps.cards import HAND_SIZE, RANK_CHARS, make_card
from prlps.equity import equity
...
def hand_shapes():
    """Every six card hand as a sorted rank multiset, no rank more than four."""
...
def deal(left, right):
    ...
    Any assignment does, since no part of Prolapse reads a suit; this one just
